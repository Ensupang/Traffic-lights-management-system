...
def CheckInput(ans):#check the format
    ...
    if get == None:
	#print
        sys.stderr.write('Error:Missing space!\n')
        result = 0
    elif get2 == None:
	#print
        sys.stderr.write('Error:Missing double quotation marks!\n')
        result = 0
    if ans[1] != ' ':
        #print 'Error:Wrong instruction!'
	sys.stderr.write('Error:Wrong instruction!\n')
        result = 0
    for i in range(1, len(ans), 1):
		...
		sys.stderr.write('Error:White-space must be in front of the first coordinate!\n')
                result = 0
            break
    if i >= len(ans):
        #print 'Error:No bracket!'
	sys.stderr.write('Error:No bracket!\n')
        result = 0
    elif ans[i] == ')':
...
def CheckSameName(ans):
    ...
    for i in range(0,len(ans),1):
		...
		sys.stderr.write('Error:Same street name!\n')
                return 0
		    ...
		    sys.stderr.write('Error:Two different streets have the same coordinates!\n')
                    return 0
...
def a(ans):#check the content and process the data
    ...
    for i in range(0,len(ans),1):
		...
		sys.stderr.write('Error:Missing comma in coordinate!\n')
                return
            elif count2 > (count+1)/2:
                #print 'Error:Comma out of coordinate!'
		sys.stderr.write('Error:Comma out of coordinate!\n')
                return
    for i in range(3,len(ans),1):
		...
		sys.stderr.write('Error:Alphabetical and space characters in street name only!\n')
                return
        else:
            IP.append(''.join(SN))
            break
    ...
    for k in range(0,len(CO)-1,1):
	    ...
	    sys.stderr.write('Error:Duplicate coordinate!\n')
            return
    if len(Pair) == 0:
        #print 'Error:Wrong coordinate(must be number)!'
	sys.stderr.write('Error:Wrong coordinate(must be number)!\n')
    elif count2 != (count/2):
        #print 'Error:Missing comma!'
	sys.stderr.write('Error:Missing comma!\n')
    elif len(CO) == 1:
        #print 'Error:A street must be more than two coordinates!'
	sys.stderr.write('Error:A street must be more than two coordinates!\n')
    elif len(Pair)%2 == 1:
        #print 'Error:Wrong coordinate(the coordinate must be in pair)!'
	sys.stderr.write('Error:Wrong coordinate(the coordinate must be in pair)!\n')
    elif len(CO)*2 > count:
        #print 'Error:Missing bracket!'
	sys.stderr.write('Error:Missing bracket!\n')
    else:
...
def r(ans,g):
    ...
    if len(g)>0:
	    ...
	    sys.stderr.write('Error:Please give me a street name!\n')
            return 1
        for k in range(0,len(g),1):
            if str.lower(g[k][0])==str.lower(b[0]):
                g.pop(k)
                break
        if len(g)==count:
            #print "Error:No this street!"
	    sys.stderr.write('Error:No this street!\n')
            return 1
    else:
        #print "Error:No street!"
	sys.stderr.write('Error:No street!\n')
        return 1
def StrtoFlo(ans):
    fi=[]
    mi = []
        ...
        for k in range(0,len(a)-1,1):
            ...
            for h in range(k+1,len(a),1):
                if a[k]==a[h]:
                    ...
                    break
        b.append(a[len(a)-1])
        #sys.stdout.write("V %d"%(len(b))
	print"V %d"%(len(b))
	#sys.stderr.write("V")
	#sys.stderr.write(len(b))
        #sys.stdout.write("E {")
	    ...
	    ss = "E {"
            for e in range(0,len(Middle),1):
                #sys.stdout.write("<%d,%d>"%((Middle[e][0]-1),(Middle[e][1])-1))
		ss += "<%d,%d>"%((Middle[e][0]-1),(Middle[e][1])-1)	
		if e!=(len(Middle))-1:
			ss += ","
	    ss += "}"
	    print ss
...
def main():
    ...
    while True:
	    ...
	    sys.stderr.write('Error:The instruction doesn\'t exist\n')
    sys.exit(0)
